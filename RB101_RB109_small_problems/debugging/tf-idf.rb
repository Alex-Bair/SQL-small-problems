...
When calculating the IDF on line 26, the original code performed integer division of the number of documents divided by the number of document with the specified term, then take the natural log of the result. However, performing integer division loses out on necessary decimal places to perform proper IDF calculations.
By multiplying the numerator `number_of_documents` by 1.0, we change the `number_of_documents` object from an Integer to a Float, and we perform much more accurate Float division.
...
=begin
If we search for a term that does not exist in any of the documents, we end up getting NaN for the TF-IDF value. This can be resolved by returning 0 in the #idf method if `number_of_documents_with_term` is equal to 0.
=end
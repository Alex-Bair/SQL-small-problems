...
-  Output: List of digits in the positive integer.
...
- Output list must be in the same sequence as the input integer.
- Duplicate values are kept.
- Output array must be comprised of integers, not strings.
...
1. Is there a maximum size limit on the integer?
2. Confirm zero is not a valid argument.
3.
...
A positive integer is split into it's individual digits. These digits are saved into an array. The array is printed out.
...
- No obvious Numeric method for splitting digits of an Integer. Plan to use Integers, Strings, and Arrays. Convert Integer to String, split into an array, convert back to integers, place into an array.
...
def digit_list(pos_int)
  pos_int.to_s.chars.collect! {|x| x.to_i}
end

...
// In both for loops, we tried to defined variables length. However in the inner for loop, we aren't declaring a new `length` variable, but rather overriding and reassigning the existing `length` variable. This causes the outer interation to be cut short.
...
The error from the code is occurring because the Mailing module is not included in the PostalService class definition, so the Mailing#send method is not available when called on the last line of the program.

This can be resolved by including the Mailing module in the PostalService class definition.

It's not a wise choice to override an important method like Object#send however, so Mailing#send should also be renamed to avoid this override.
...
module Mailing
  ...
  def custom_send(destination, mail)
...
puts johns_postal_service.custom_send(ellens_postal_service.street_address, Postcard.new('Greetings from Silicon Valley!'))
